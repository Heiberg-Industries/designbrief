...
export async function handleGetTokens({ style, format }) {
  try {
    ...
    if (format === 'dtcg') {
      const { generateDtcg } = await import('./dtcg.mjs');
      const dtcg = generateDtcg(parsed);
      return {
        content: [{ type: 'text', text: JSON.stringify({ style, ...dtcg }, null, 2) }],
      };
    }

    if (format === 'css') {
      const { generateCss } = await import('./css.mjs');
      return {
        content: [{ type: 'text', text: generateCss(parsed) }],
      };
    }
